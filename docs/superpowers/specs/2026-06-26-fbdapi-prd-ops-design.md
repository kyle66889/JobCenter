...
> **关键设计：id 由调用方决定，不在 api 里读 config.sh。**
> `FEDEX_FUEL_*_ID` 由**生成任务的 fuel 脚本**（调用方）在 push 待处理任务时读取，并写进任务的 `target_ids` 列（见第 4.1 节）。
> `FbdPrdService` 只接收传入的 ids、不碰 config.sh。好处：ids 在"生成任务那一刻"定死并随任务留痕，
> 之后改 config.sh 不影响已存在的待审批任务。

## 4.1 FbdTask 新增列 `target_ids`

- `back/data/fbdTask.ts`：模型加字段 `target_ids?: any`（`DataTypes.JSON`，列名 `target_ids`），构造器默认 `{}`。
- `back/loaders/db.ts` 的 `migrations` 数组加一项给已存在的表补列：
  `{ table: 'FbdTasks', column: 'target_ids', type: 'JSON' }`。
- 内容形如（fuel 任务）：`{"ground":"123,124","express":"456"}`（直接存 config.sh 里的逗号串，由 api 侧 `parseIds` 解析）。
- fuel 脚本 `push_fbd_task`（`docker/data/scripts/fedex_fuel_surcharge.py`）：从 `os.environ` 读
  `FEDEX_FUEL_GROUND_ID` / `FEDEX_FUEL_EXPRESS_ID`，组装 `target_ids` JSON，写进 INSERT 的新列。
  （脚本运行在容器内，config.sh 的 export 即进程 env）
...
- `public async apply(type: string, payload: any, targetIds: any): Promise<string>`：按任务类型分发。
  `targetIds` 由调用方（approve）从任务的 `target_ids` 列取出传入。
  - `case 'fedex_fuel_charge'` → `updateFuelSurcharge(payload, targetIds)`
  - `default` → `throw new Error('未知任务类型: ' + type)`
- `public async updateFuelSurcharge(payload: any, targetIds: any): Promise<string>`：见第 6 节。

## 6. updateFuelSurcharge 逻辑

纯逻辑抽到 `back/shared/fbdFuel.ts`（可单测），DB 执行留在 service。**api 不读 config.sh**，ids 全部来自传入的 `targetIds`。

- `pctToFraction(s: string): number`：去 `%`、转数值、`/100`。`"16.50%" → 0.165`、`"17.00%" → 0.17`、`"0%" → 0`；
  解析不出数值 → 抛错。（结果按 4 位小数处理，避免浮点尾差）
- `parseIds(val: string): string[]`：逗号分隔 → trim → 去空项。`""` / `undefined` → `[]`。
- `buildFuelUpdates(payload, targetIds): Array<{ label:string; rate:number; ids:string[] }>`：
  `targetIds` 形如 `{ground:"123,124", express:"456"}`。组装两条（ground 用 `payload.ground` + `parseIds(targetIds.ground)`，
  express 用 `payload.express_package` + `parseIds(targetIds.express)`），`ids` 为空的那条**跳过**（不进结果）。纯函数，单测。
...
- 两条都因 id 为空被跳过 → 抛错 `"任务未携带任何 MZL_Priceid（target_ids 为空）"`。

## 7. 接入 approve + 移除占位

- `back/shared/fbd.ts`：删除占位 `applyUpdate`（及其 2 条单测），保留 `FbdTaskStatus` / `FBD_STATUS_LABEL` / `assertApprovable`。
- `back/services/fbd.ts`：`approve` 不再 import `applyUpdate`；改为注入 `FbdPrdService`，
  调用 `this.fbdPrd.apply(doc.type, doc.payload, doc.target_ids)`（把任务的 `target_ids` 列传给 api）。
...
- `parseIds`：`"123,124"→["123","124"]`、`" 1 , ,2 "→["1","2"]`、`""→[]`、`undefined→[]`。
- `buildFuelUpdates`：`targetIds` 两条都有 id；仅 ground；仅 express；都空（返回空数组）。
...
- 改 `back/services/fbd.ts`（approve 调 FbdPrdService，传 target_ids）
- 改 `back/data/fbdTask.ts`（加 `target_ids` JSON 字段）
- 改 `back/loaders/db.ts`（migrations 给 FbdTasks 补 `target_ids` 列）
- 改 `docker/data/scripts/fedex_fuel_surcharge.py`（push_fbd_task 读 env id 写入 target_ids）
