...
> `FEDEX_FUEL_*_ID` 由**生成任务的 fuel 脚本**（调用方）在 push 待处理任务时读取，并写进任务的 `MZL_PriceID` 列（见第 4.1 节）。
> `FbdPrdService` 只接收传入的 ids、不碰 config.sh。好处：ids 在"生成任务那一刻"定死并随任务留痕，
> 之后改 config.sh 不影响已存在的待审批任务。

## 4.1 FbdTask 新增列 `MZL_PriceID`

- `back/data/fbdTask.ts`：模型加字段 `MZL_PriceID?: any`（`DataTypes.JSON`，列名 `MZL_PriceID`），构造器默认 `{}`。
...
- fuel 脚本 `push_fbd_task`（`docker/data/scripts/fedex_fuel_surcharge.py`）：从 `os.environ` 读
  `FEDEX_FUEL_GROUND_ID` / `FEDEX_FUEL_EXPRESS_ID`，组装 `MZL_PriceID` JSON，写进 INSERT 的新列。
...
- `public async apply(type: string, payload: any, mzlPriceIds: any): Promise<string>`：按任务类型分发。
  `mzlPriceIds` 由调用方（approve）从任务的 `MZL_PriceID` 列取出传入。
  - `case 'fedex_fuel_charge'` → `updateFuelSurcharge(payload, mzlPriceIds)`
  - `default` → `throw new Error('未知任务类型: ' + type)`
- `public async updateFuelSurcharge(payload: any, mzlPriceIds: any): Promise<string>`：见第 6 节。

## 6. updateFuelSurcharge 逻辑

纯逻辑抽到 `back/shared/fbdFuel.ts`（可单测），DB 执行留在 service。**api 不读 config.sh**，ids 全部来自传入的 `mzlPriceIds`。
...
- `buildFuelUpdates(payload, mzlPriceIds): Array<{ label:string; rate:number; ids:string[] }>`：
  `mzlPriceIds` 形如 `{ground:"123,124", express:"456"}`。组装两条（ground 用 `payload.ground` + `parseIds(mzlPriceIds.ground)`，
  express 用 `payload.express_package` + `parseIds(mzlPriceIds.express)`），`ids` 为空的那条**跳过**（不进结果）。纯函数，单测。
...
- 两条都因 id 为空被跳过 → 抛错 `"任务未携带任何 MZL_Priceid（MZL_PriceID 为空）"`。
...
- `back/services/fbd.ts`：`approve` 不再 import `applyUpdate`；改为注入 `FbdPrdService`，
  调用 `this.fbdPrd.apply(doc.type, doc.payload, doc.MZL_PriceID)`（把任务的 `MZL_PriceID` 列传给 api）。
...
- `buildFuelUpdates`：`mzlPriceIds` 两条都有 id；仅 ground；仅 express；都空（返回空数组）。
...
- 改 `back/services/fbd.ts`（approve 调 FbdPrdService，传 MZL_PriceID）
